...
# build directory to their legacy location at Build/StorageRoomKit
# under the project root. This is the include path for Xcode
# 3 projects and Xcode 4 projects not using derived data

IFS=$'\n'
if [ -d "${TARGET_BUILD_DIR}/include/StorageRoomKit" ]; then
    rsync -av --delete "${TARGET_BUILD_DIR}/include/StorageRoomKit" "${SOURCE_ROOT}/Build"
else
